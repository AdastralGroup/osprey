...
{  // yucky hack but we can manually start palace from godot now - means we don't hang at
   // the godot splash
...
{
  A_init_error_system();
  A_error_system->register_listener(
      EventType::kOnError,
      [this](Event& ev) { emit_signal("error", String(static_cast<ErrorMessage&>(ev).get_message().c_str())); });
...
godot::String binding::get_sourcemod_path() { return p->M_sourcemods_path.c_str(); };
