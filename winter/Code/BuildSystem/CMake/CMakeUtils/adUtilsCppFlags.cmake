...
function(ad_check_build_type)
    # set the default build type
    if (NOT CMAKE_BUILD_TYPE)
        set(CMAKE_BUILD_TYPE ${AD_BUILDTYPENAME_DEV} CACHE STRING "Choose the type of build, options are: None ${AD_BUILDTYPENAME_DEBUG} ${AD_BUILDTYPENAME_DEV} ${AD_BUILDTYPENAME_RELEASE}." FORCE)
    endif ()
endfunction()

# #####################################
# ## ad_set_build_flags_msvc(<target>)
# #####################################
function(ad_set_build_flags_msvc TARGET_NAME)
    ...
    if ((CMAKE_SIZEOF_VOID_P EQUAL 4) AND AD_CMAKE_ARCHITECTURE_X86)
    ...
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_RELEASE_UPPER}>:/Zo>")
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_DEV_UPPER}>:/Zo>")

    # /Ob1: Only consider functions for inlining that are marked with inline or forceinline
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_DEBUG_UPPER}>:/Ob1>")

    # /Ox: favor speed for optimizations
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_RELEASE_UPPER}>:/Ox>")

    # /Ob2: Consider all functions for inlining
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_RELEASE_UPPER}>:/Ob2>")

    # /Oi: Replace some functions with intrinsics or other special forms of the function
    target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${AD_BUILDTYPENAME_RELEASE_UPPER}>:/Oi>")
    ...
    if (CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND AD_CMAKE_ARCHITECTURE_X86)
    ...
    if (${ARG_NO_DEBUG})
        ...
        set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${AD_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_RELEASE})
        set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${AD_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_RELEASE})
    else ()
        set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${AD_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_DEBUG})
        set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${AD_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_${AD_DEV_BUILD_LINKERFLAGS}})
    endif ()

    set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${AD_BUILDTYPENAME_RELEASE_UPPER} ${LINKER_FLAGS_RELEASE})

    if (AD_ENABLE_COMPILER_STATIC_ANALYSIS)
    ...
    # Disable deprecation warnings (godot, etc)
    target_compile_options(${TARGET_NAME} PRIVATE /wd4996)

endfunction()

# #####################################
# ## ad_set_build_flags_clang(<target>)
# #####################################
function(ad_set_build_flags_clang TARGET_NAME)
    ...
    if (AD_ENABLE_QT_SUPPORT)
        # Ignore any warnings caused by Qt headers
        target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${AD_QT_DIR}\"")
    endif ()

    # Ignore any warnings caused by headers inside the ThirdParty directory.
    if (AD_SUBMODULE_PREFIX_PATH)
        target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CMAKE_SOURCE_DIR}/${AD_SUBMODULE_PREFIX_PATH}/Code/ThirdParty\"")
    else ()
...
# #####################################
# ## ad_set_build_flags_gcc(<target>)
# #####################################
function(ad_set_build_flags_gcc TARGET_NAME)
    ...
    if (NOT (CMAKE_CURRENT_SOURCE_DIR MATCHES "Code/ThirdParty"))
        ...
        # attributes = error if a attribute is placed incorrectly (e.g. AD_FOUNDATION_DLL)
...
function(ad_set_build_flags TARGET_NAME)
    ...
    if (AD_CMAKE_COMPILER_MSVC)
        ad_set_build_flags_msvc(${TARGET_NAME} ${ARGN})
    endif ()

    if (AD_CMAKE_COMPILER_CLANG)
        ad_set_build_flags_clang(${TARGET_NAME} ${ARGN})
    endif ()

    if (AD_CMAKE_COMPILER_GCC)
        ad_set_build_flags_gcc(${TARGET_NAME} ${ARGN})
    endif ()
endfunction()
