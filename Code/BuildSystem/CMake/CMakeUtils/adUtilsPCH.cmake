...
# #####################################
# ## ad_retrieve_target_pch(<target> <pch-name>)
# #####################################
function(ad_retrieve_target_pch TARGET_NAME PCH_NAME)
...
# #####################################
# ## ad_pch_use(<pch-header> <cpp-files>)
# #####################################
function(ad_pch_use PCH_H TARGET_CPPS)
	if(NOT AD_USE_PCH OR AD_CMAKE_GENERATOR_MSVC)
...
# #####################################
# ## ad_pch_create(<pch-header> <cpp-file>)
# #####################################
function(ad_pch_create PCH_H TARGET_CPP)
	if(NOT AD_CMAKE_GENERATOR_MSVC)
		return()
	endif()
...
# #####################################
# ## ad_find_pch_in_file_list(<files> <out-pch-name>)
# #####################################
function(ad_find_pch_in_file_list FILE_LIST PCH_PATH PCH_NAME)
...
# #####################################
# ## ad_auto_pch(<target> <files> [<file-exclude-regex> ... ])
# #####################################
function(ad_auto_pch TARGET_NAME FILES)
	foreach(EXCLUDE_PATTERN ${ARGN})
		list(FILTER FILES EXCLUDE REGEX ${EXCLUDE_PATTERN})
	endforeach()

	ad_find_pch_in_file_list("${FILES}" PCH_PATH PCH_NAME)
	
	if(NOT PCH_NAME)
		return()
	endif()

	ad_pch_create("${PCH_NAME}.h" ${PCH_PATH})

	ad_pch_use("${PCH_NAME}.h" "${FILES}")

	ad_set_target_pch(${TARGET_NAME} ${PCH_NAME})
endfunction()
